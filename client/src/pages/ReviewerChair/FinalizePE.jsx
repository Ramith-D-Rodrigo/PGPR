...
function FinalizePE() {
    ...
    useEffect(() => {
      document.title = "End the Proper Evaluation process";
    ...
    const headerInfo = [
      ...
      const createData = (reviewer,designation,role,progress) => {
        const actions = <Button variant="contained" color="primary"><Link to={`../view_PE_progress/${pgprId}/${reviewer.id}`}>View</Link></Button>
        return {name:reviewer.name,designation,role,progress,actions}
      }

      const rows = [
        createData({id:1,name:"Dr. K. K. K. Perera"},"Senior Lecturer","Chair","Completed"),
        createData({id:1,name:"Dr. K. K. K. Perera"},"Senior Lecturer","Reviewer","Evaluating"),
        createData({id:1,name:"Dr. K. K. K. Perera"},"Senior Lecturer","Reviewer","Evaluating"),
      ]
    return (
      <>
            ...
            <Divider style={{margin:"2rem 0 1rem"}} textAlign="center">Proper Evaluation Progress</Divider>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between',margin:"1rem 0" }}>
                ...
                >
                    Proper Evaluation Period : <strong>2023 AUG 12 To 2023 AUG 12</strong>
                </Typography>
                <Typography
                    variant="button"
                    style={{ margin: "0 0 0 20px" }}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', width: '100%', padding: '20px 0',height:"auto" }}>
                <Link to={`../End_PE/${pgprId}`}><Button variant="contained" size="small" style={{width:"300px",height:'55px',backgroundColor:"#A2CBEA",color:'black'}}>End the Proper Evaluation</Button></Link>
            </Box>
      </>
    )
}
