                ...
                <Box sx={{ ...secondaryBoxStyle, borderLeft: "1px" }}>
                    ...
                    <Box sx={detailedBox}>
                        <Box sx={itemBox}>
                            ...
                            </Box>
                            {
                                ...
                                <Box align='left' sx={{ display: 'flex', flexDirection: 'column' }}>
                                    <Button variant='outlined' size='small' sx={{ mb: 1 }} color="primary" disabled={status?.toLowerCase() === 'accepted'} onClick={() => handleClickOpen()}>Select</Button>
                                    <Button variant='outlined' size='small' sx={{ mb: 1 }} color="primary" disabled={status?.toLowerCase() === 'accepted'} onClick={() => handleClickOpen()}>Select</Button>
                                    <Button variant='outlined' size='small' sx={{ mb: 1 }} color="primary" disabled={status?.toLowerCase() === 'accepted'} onClick={() => handleClickOpen()}>Select</Button>
                                </Box>
                            }
                        ...
                        {(auth.authRole[0] === 'qac_officer' || auth.authRole[0] === 'qac_director') &&
                            <Box sx={itemBox}>
                                <Box align='left'>
                                    <Button variant='outlined' size='small' sx={{ mb: 1 }} color="primary" disabled={status?.toLowerCase() === 'accepted'} onClick={() => handleSubmitReviewTeam()}>Submit Review Team to Dean</Button>
                                </Box>
                                <Box align='left'>
                                    <Button variant='outlined' size='small' sx={{ mb: 1 }} color="primary" disabled={status?.toLowerCase() === 'accepted'} onClick={() => handleRemoveReviewTeam()}>Remove Review Team</Button>
                                </Box>
                            </Box>
                        }
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', width: '100%', mt: 2 }}>
                    {auth?.authRole[0] === 'dean' && <Button variant="contained" color="primary" sx={{ width: "300px" }}>Submit Consent Letter</Button>}
                    {
                        (auth?.authRole[0] === 'qac_officer' || auth?.authRole[0] === 'qac_director' || auth?.authRole[0] === 'reviewer') && pgpr?.statusOfPgpr !== 'PLANNING' ?
                            <Button variant="contained" color="primary" sx={{ width: "300px" }} component={Link} to={`/${auth.authRole[0]}/pgprs/${pgprId}/ser/${pgpr?.selfEvaluationReport?.id}`}>
